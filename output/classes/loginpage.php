...
class LoginPage extends RunnerPage
{
	...
	protected function logInHardcoded( $pUsername, $pPassword, $skipCheck = false, $fireEvents = true )
	{
		...
		if( $skipCheck || $this->checkUsernamePassword( $pUsername, $pPassword ) )
		{
			...
			$dummy = array();
			SetAuthSessionData($pUsername, $dummy, $pPassword, $this, $fireEvents );
			return true;
		}
		if( $fireEvents )
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$sqlHead = "select * from ".$this->connection->addTableWrappers( "" ) ." where ";
		$sql = array();
		if( $userIdField ) {
			$sql []= $sqlHead
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "", $data, $blobfields, $this );
			if( !$retval )
				return false;
