...
function checkTableName($shortTName, $type=false)
{
	...
	if ("check_constraints" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
...
function GetTablesList($pdfMode = false)
{
	...
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("INFORMATION_SCHEMA.CHECK_CONSTRAINTS");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
		$arr[]="INFORMATION_SCHEMA.CHECK_CONSTRAINTS";
	}
	$tableAvailable = true;
	if( $checkPermissions ) {
		$strPerm = GetUserPermissions("dbo.user");
		$tableAvailable = ( strpos($strPerm, "P") !== false
			|| $pdfMode && strpos($strPerm, "S") !== false );
	}
	if( $tableAvailable ) {
...
function GetUserPermissionsStatic( $table )
{
	...
	if( $table=="INFORMATION_SCHEMA.CHECK_CONSTRAINTS" )
	{
//	default permissions
